...
//            reader.readNext();
            ...
            while ((nextLine = reader.readNext()) != null) {
                try {
                    ...
                    Long idKecmatan = Long.valueOf(nextLine[2].trim());
                    ...
                    if (existingKecamatan.isPresent()) {
                        ...
                        kelurahan.setId_kelurahan(idKecmatan);
                        kelurahan.setNameKelurahan(namaKelurahan);
                    }

                    Optional<Kecamatan> optionalKecamatan = kecamatanRepository.findById(idKecmatan);
                    if (optionalKecamatan.isPresent()) {
                        kelurahan.setKecamatan(optionalKecamatan.get());
                    } else {
                        log.warn("Kelurahan dengan ID {} tidak ditemukan.", idKelurahan);
                        continue;
                    }
                    ...
                    log.info("Data valid: ID={} Nama={} Kelurahan={}", idKecmatan, namaKelurahan, idKelurahan);

                } catch (NumberFormatException e) {
                    log.error("Format ID tidak valid: {} - Error: {}", nextLine, e.getMessage());
                } catch (Exception e) {
                    throw new RuntimeException("Gagal memproses Kelurahan: " + e.getMessage(), e);
                }
            }
            ...
            return "Import Kelurahan berhasil! Total data yang disimpan: " + kelurahanList.size();

        } catch (IOException e) {
            log.error("Gagal membaca file Kelurahan! Error: {}", e.getMessage(), e);
            return "Gagal membaca file Kelurahan!";
        } catch (CsvValidationException e) {
            log.error("Kesalahan validasi Kelurahan! Error: {}", e.getMessage(), e);
            return "Gagal memvalidasi Kelurahan!";
        }
